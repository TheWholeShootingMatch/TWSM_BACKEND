...
router.post("/login", function (req, res, next) {
    User.findOne({ id: req.body.id }, function (err, user) {
        if (err) {
            ...
            console.log("아이디 없음1");
            return res.send({ name: "" }); //id ===""일 경우 변화 없음..
        } else {
            ...
            if (status) {
                ...
                return res.send({ name: "" }); //blocked user일 경우
                ...
                return res.send({ name: req.session.user_name });
            } else {
...
router.post("/fav_model", async function (req, res, next) {
    const model = await User.findOne({ _id: req.session.user_Oid, fav_models: req.body.id });
...
router.post("/fav_photographer", async function (req, res, next) {
    const model = await User.findOne({ _id: req.session.user_Oid, fav_photographers: req.body.id });
